...
model = load_model('./Final_Model/all_cities_lstm_model_v2.h5')
...
def predict():
    try:
        ...
        predicted_inflation_python = float(predicted_inflation[0])

        # Inverse transform the prediction to get the actual value
        predicted_inflation_actual = scaler.inverse_transform(np.array([[predicted_inflation_python]]))

        # Use [0][0] to extract the actual value
        prediction = round(predicted_inflation_actual[0][0], 2)
        ...
        months_to_goal = int((goal - prediction) / monthly_savings)
